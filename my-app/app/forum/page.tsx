...
export default function ForumPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] bg-gradient-to-b from-white to-[#DEEAE5]/30 p-6 md:p-8">
      <h1 className="text-2xl md:text-3xl font-bold text-[#473F63] mb-6">
        Forum
        <span className="block text-base font-normal text-[#1E4D57]/80 mt-2">
      ...
      <Tabs defaultValue="groups" className="space-y-6">
        ...
        <TabsContent value="groups" className="grid gap-6">
          {groups.map((group) => (
              ...
              <Card className="group hover:shadow-lg transition-all duration-300 border-none bg-white/80 backdrop-blur-sm">
                <div className="p-4">
                  {group.unreadCount && (
                    <Badge 
                      variant="secondary" 
                      className="w-fit mb-2 bg-[#E6E3FD] text-[#473F63] font-medium"
                    >
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-[#473F63] font-medium text-xl group-hover:text-[#1E4D57] transition-colors">
                        {group.name}
                      </h3>
                      <p className="text-[#473F63]/90 text-sm mt-1 line-clamp-2">
                        {group.description}
                      </p>
                      <div className="flex items-center gap-4 mt-2 text-sm text-[#473F63]/80 group-hover:text-[#473F63]">
          ...
          <div className="mt-6">
            <h2 className="text-xl font-semibold text-[#473F63] mb-4">Trending Discussions</h2>
            {discussions.map((discussion) => (
              <Link key={discussion.id} href={`/forum/chat/discussion${discussion.id}`}>
                <Card className="group hover:shadow-lg transition-all duration-300 border-none bg-white/80 backdrop-blur-sm mb-4">
                  <div className="p-4">
                    <div className="flex items-center gap-4">
                      <img
        ...
        <TabsContent value="private" className="grid gap-6">
          {[
            {
              id: '1',
